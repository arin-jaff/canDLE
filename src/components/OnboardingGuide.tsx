...
const STEPS = [
  ...
  },
  {
    ...
    visual: (
      <div className="border border-terminal-border p-3 bg-terminal-dark">
        <div className="flex gap-1 mb-2">
          {['1M', '1Y'].map(t => (
            <div key={t} className="px-3 py-1 text-[10px] font-mono text-terminal-green bg-terminal-green-dark/30 border border-terminal-green/30">{t}</div>
          ))}
          {['5Y', 'ALL'].map(t => (
            <div key={t} className="px-3 py-1 text-[10px] font-mono text-terminal-border border border-terminal-border">{t} {'\u25A0'}</div>
          ))}
        </div>
  ...
  },
  {
    ...
    visual: (
      <div className="border border-terminal-border bg-terminal-dark text-[10px] font-mono">
        {[
          ...
          <div key={h.label} className={`flex items-center px-3 py-2 border-b border-terminal-border last:border-b-0 ${h.val ? 'bg-terminal-green-dark/20' : ''}`}>
            <span className="text-terminal-muted uppercase tracking-wider w-24">{h.label}</span>
  ...
  },
  {
    ...
    visual: (
      <div className="space-y-2">
        <div className="border border-terminal-border bg-terminal-panel px-3 py-2 text-xs font-mono text-terminal-muted flex items-center gap-2">
          <span className="text-terminal-border">{'>'}</span>
          <span>Search for a ticker...</span>
        </div>
        <div className="flex gap-2 text-[10px] font-mono">
          <div className="px-2 py-1 border border-terminal-red text-terminal-red">MSFT {'\u2718'}</div>
          <div className="px-2 py-1 border border-terminal-red text-terminal-red">GOOG {'\u2718'}</div>
          <div className="px-2 py-1 border border-terminal-green text-terminal-green">AAPL {'\u2714'}</div>
        </div>
      </div>
    ),
  },
      ...
      <div className="flex items-center justify-center py-3">
        <div className="grid grid-cols-4 gap-px bg-terminal-border text-center">
          {[
            ...
            <div key={s.l} className="bg-terminal-panel px-3 py-2">
              <div className="text-lg font-mono font-bold text-terminal-green">{s.v}</div>
              <div className="text-[7px] text-terminal-muted tracking-widest uppercase">{s.l}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80" onClick={onComplete}>
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-terminal-yellow px-4 py-3 bg-terminal-dark">
          <div>
            <div className="text-[11px] text-terminal-yellow tracking-widest uppercase font-medium">
              {current.title}
            </div>
            <div className="text-[9px] text-terminal-muted tracking-wider">{current.subtitle}</div>
          </div>
          ...
          <div className="flex gap-2">
            {step > 0 && (
              <button
                ...
                className="flex-1 px-4 py-2.5 text-[10px] uppercase tracking-widest
