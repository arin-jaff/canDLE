...
export function AdminPanel({ currentPuzzleId, onLoadTicker, onReset, onClose }: AdminPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-start justify-center bg-black/70 overflow-y-auto py-8" onClick={onClose}>
      <div
        className="w-full max-w-xl border border-terminal-yellow bg-terminal-black mx-4"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-terminal-yellow px-4 py-3 sticky top-0 bg-terminal-dark z-10">
          <div className="flex items-center gap-3">
            <span className="text-[11px] text-terminal-yellow tracking-widest uppercase font-medium">
              ADMIN PANEL
            </span>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="border-t border-terminal-border pt-4">
            <button
              onClick={onReset}
              className="w-full px-4 py-2.5 text-xs uppercase tracking-widest
